...
| `migration_manifest.json` | LVS-owned snake_case contract | Private, explicitly not-public-safe migration-bundle manifest. The compatibility v1 writer remains available. Contract v2 combines the A1 logical inventory, integrity/privacy metadata, safe transaction engine, and frontend-neutral plan with A2 semantic portable-settings, active custom/modified-stock profile, menu-group, and setup-history migration. Results, archived profiles, sensor logs, credentials, and active hardware-validation state remain excluded. Historical v1 bundles are conservatively adapted into the same settings/history semantics; compatibility fixtures carrying the explicit legacy-restore marker retain legacy restore behavior. |
