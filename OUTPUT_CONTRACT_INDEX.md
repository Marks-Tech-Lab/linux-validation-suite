...
| `migration_manifest.json` | LVS-owned snake_case contract | Private, explicitly not-public-safe migration-bundle manifest. The compatibility v1 writer remains available. Contract v2 provides the A1 logical inventory, per-entry schemas, hashes/sizes, privacy markers, safe transaction engine, and frontend-neutral planning foundation; A2 semantic settings/profile/history handlers are not yet operational. V1 bundles can be projected through the conservative v2 adapter without changing legacy v1 restore behavior. |
